  ...
  getProductDetails(): void {
    this.productService.getDetailProduct(this.productId).subscribe({
      next: (product: Product) => {
        ...
        if (this.updatedProduct.product_images.length > 0) {
          this.updatedProduct.thumbnail = this.updatedProduct.product_images[0].image_url || ''; // Gán giá trị tên ảnh
        } else {
          this.updatedProduct.thumbnail = ''; // Gán giá trị mặc định nếu không có hình ảnh
        }              
